...
OUTPUT = Path(__file__).parent / "MNACP_Bitirme_Kitapcik_v4.docx"
...
def build_bolum1(doc):
    ...
    add_bullet(doc, "Bir ajanın orkestratörü atlatıp doğrudan başka bir ajana görev devredebildiği peer delegasyon mekanizması; orkestratör bağımlılığı olmadan herhangi iki ajanın birbirine yetkinlik bazlı görev devredebildiği genel BaseAgent._delegate_to_peer() API'si ve canlı görselleştirmesi.")
...
def build_bolum3(doc):
    ...
    add_table_simple(
        ...
        [
            ...
            ["SearchAgent", "9002", "web_search, fetch_page, summarize, extract_links", "DuckDuckGo HTML scraping (gerçek sonuç, API key yok); analiz gerektiren görevlerde BaseAgent._delegate_to_peer() ile AnalysisAgent'a otomatik yönlendirme"],
    ...
    add_body(
        doc,
        "İlk tasarımda peer delegasyon DataAgent ve SearchAgent'a ayrı ayrı "
        "hardcode edilmişti: her ajan kendi _delegate_to_analysis_agent() "
        "metodunu barındırıyor, AnalysisAgent'ı adına göre arıyordu. Bu "
        "yaklaşımın iki sorunu vardı: (a) yeni bir ajan eklendiğinde "
        "delegasyon mantığını sıfırdan yazmak gerekiyordu; (b) sisteme yeni "
        "bir uzman (örneğin no-code ile oluşturulmuş bir GenericAgent) "
        "eklendiğinde, DataAgent ve SearchAgent onu keşfedemiyor ve "
        "delegasyon hâlâ yalnızca AnalysisAgent'a gidiyordu.",
    )
    add_body(
        doc,
        "Bu sınırlamayı gidermek için peer delegasyon mantığı BaseAgent "
        "soyut sınıfına taşınmış ve _delegate_to_peer(task, context, "
        "required_capabilities, peer_context) adlı genel bir metod olarak "
        "yayımlanmıştır. Metod, required_capabilities listesini karşılayan "
        "herhangi bir ajana HTTP delegasyonu yapar; discovery, zincir yönetimi "
        "ve hata işleme tamamen BaseAgent içinde encapsulate edilmiştir. "
        "DataAgent ve SearchAgent artık bu tek metodu çağırır; AnalysisAgent'a "
        "özgü herhangi bir referans bulunmamaktadır. Böylece sisteme "
        "generate_report veya trend_analysis yetkinliğine sahip herhangi bir "
        "ajan (no-code GenericAgent dahil) eklendiğinde, mevcut peer "
        "delegation akışı ek kod değişikliği gerektirmeksizin bu yeni ajanı "
        "otomatik keşfedip ona yönlendirebilir hale gelmiştir.",
    )
...
def build_bolum4_algoritmalar(doc):
    ...
    add_body(doc, "Sözde-kod (BaseAgent._delegate_to_peer() genel API'si ile güncellenmiş hali):")
    pseudo3 = (
        ...
        "  9:         // BaseAgent._delegate_to_peer() — herhangi bir capability'ye\n"
        " 10:         // sahip herhangi bir ajana yönlendirme; AnalysisAgent'a\n"
        " 11:         // özgü referans yok, GenericAgent da seçilebilir\n"
        " 12:         peer ← self._delegate_to_peer(\n"
        " 13:                   task='trend analizi ve rapor oluştur',\n"
        " 14:                   context=context,\n"
        " 15:                   required_capabilities=[generate_report, trend_analysis],\n"
        " 16:                   peer_context={values, statistics, column, ...})\n"
        " 17:         return {statistics, analysis: peer.result,\n"
        " 18:                 _peer_delegations: [peer.metadata]}\n"
        " 19:     return stats\n"
        " 20: ..."
    )
    p = doc.add_paragraph()
...
def build_bolum6_test(doc):
    ...
    add_body(
        doc,
        "Toplam 98 birim test mock'larla yazılmıştır ve dış HTTP bağlantısı "
    ...
    add_body(
        doc,
        "Son geliştirme döneminde eklenen test_registry_components.py dosyası "
        "daha önce doğrudan test edilmeyen dört modülü kapsamaktadır: "
        "TrustScorer (9 test — başarı oranı, gecikme cezası, ardışık hata "
        "üstel ceza, trend hesaplama), CapabilityEmbedder (6 test — fit/"
        "transform, benzer metin yakınlığı, top-k sıralama, exclude desteği), "
        "DiscoveryProtocol (5 test — en iyi ajan seçimi, boş sonuç işleme, "
        "istek formatı doğrulama, araç sahibi çözümleme) ve GenericAgent "
        "(7 test — registry kayıt şeması, derinlik/döngü reddi, Claude "
        "Haiku mock'u ile tool çalıştırma ve delegasyon kabul). Bu testler "
        "gerçek API anahtarı gerektirmez; Claude çağrıları unittest.mock ile "
        "taklit edilir.",
    )
